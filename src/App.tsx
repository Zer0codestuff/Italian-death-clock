...
const Hero = ({ source }: { source?: Source }) => {
  ...
  return (
    <section className="hero" id="top"><div className="hero__inner">
...
const PaygScene = ({ source }: { source?: Source }) => {
  ...
  return (
    <section className="scroll-scene scroll-scene--payg" id="story" data-scroll-scene><div className="scene-sticky scene-shell">
      ...
      <div className="payg-visual"><img src="/assets/payg-flow.png" alt={copy.payg.alt} width="1200" height="800" loading="lazy" /><div className="payg-fact"><strong>{copy.payg.fact}</strong><span>{copy.payg.factLabel}</span><small>{copy.payg.note}</small></div></div>
...
const DemographyScene = ({ source, shares }: { source?: Source; shares: Array<{ year: number; age15to64: number; age65Plus: number }> }) => {
  ...
  return (
    <section className="scroll-scene scroll-scene--demography" data-scroll-scene><div className="scene-sticky scene-shell">
      ...
      <div className="demography-visual"><img src="/assets/demographic-load.png" alt={copy.demography.alt} width="1200" height="675" loading="lazy" />
...
const SpendingScene = ({ source, points }: { source?: Source; points: SpendingPoint[] }) => {
  ...
  return (
    <section className="scroll-scene scroll-scene--spending" data-scroll-scene><div className="scene-sticky scene-shell">
      ...
      <div className="spending-visual"><SpendingPath points={points} label={copy.spending.chartAlt} language={language} /></div><TruthLine kind="official" source={source} />
    </div></section>
  );
};
...
const PersonalScene = ({ source, projections }: { source?: Source; projections: ReplacementProjection[] }) => {
  ...
  return (
    <section className="personal-scene" data-scroll-scene><div className="scene-shell personal-layout">
      ...
      <div className="personal-context" aria-live="polite"><span>{copy.personal.retirement}<strong>{result.retirementYear}</strong></span><span>{copy.personal.replacement}<strong>{number.format(result.replacementRatePercent)}%</strong></span></div>
      ...
      <p className="money-explanation">{copy.personal.equal}</p><div className="assumption-line"><TruthLine kind="scenario" /><p>{copy.personal.assumptions}</p><SourceLink source={source} compact /></div>
    </div></section>
  );
};
